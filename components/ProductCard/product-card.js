...
const ProductCard = ({
  ...
  const [selectedProductStock, setSelectedProductStock] = useState(
      product.variations ? product.variations.nodes[0].stockQuantity : product.stockQuantity,
  );
    ...
    for (const attribute of product.variations.nodes) {
      if (attribute.size?.nodes[0].value === selectedProductSize) {
        colorList.push(
            <button
                ...
                className={`${s.color} ${
                    selectedProductColor === attribute.color?.nodes[0].value ? s.active : ''
                }`}
                onClick={() => {
      ...
      sizeList.push(
          <button
              key={uuidv4()}
              className={`${s.size} ${
                  selectedProductSize === attribute.size?.nodes[0].value ? s.active : ''
  ...
  const data = [
    ...
    },
    {
      ...
      content: (
          <div>
            При заказе от 500 000 сум и дополнительного размера, обратная дорога оплачивается нами.
            <br/>
            <br/>
            В случае если сумма заказа менее 500 000 сум и потребуется два размера для примерки ,
            дорога туда и обратно оплачивается клиентом. Оплата принимается через CLICK ,PAYME и
            Наличный расчёт.
            <br/>
            <br/>
            Заказы принимаем с понедельника по субботу с 9:00 до19:00. Воскресенье: Выходной.
            <br/>
            За отказ или возврат товара оплата за доставку производиться со стороны покупателя .
          </div>
      ),
    },
  ];
      ...
      <>
                ...
                {product.galleryImages.nodes.map(({sourceUrl}) => (
                    <div className={s.img} key={sourceUrl}>
                      <LightgalleryItem src={sourceUrl} thumb={sourceUrl}>
                        <Image src={sourceUrl} width={350} height={520} alt="product_extra_images"/>
                      </LightgalleryItem>
                    </div>
                ))}
              </div>
        ...
        >
          Наличие в магазинах
            ...
            <div className={s.info}>
              <div className={s.content}>
                ...
                Доставка по городу Ташкент осуществляется через Яндекс GO( такси) в течение 48 часов
                от нашего склада до вашего дома по тарифному плану.
              </div>
              <div className={s.content}>
                <div className={s.title}>Доставка по Регионам Узбекистана в течение 2-5 дней.</div>
              </div>
            </div>
            {windowWidth <= 1023 ? (
                <>
          ...
          <div className="col-lg-12 col-12">
            <ProductSlider title="Возможно вам понравится" products={product.related.nodes}/>
          </div>
        </div>
      </>
  );
