...
const CatalogMain = ({ products, categories, category, parentCategory, pageInfo, activeTerms }) => {
  ...
  let locationSearch = typeof window !== 'undefined' ? window.location.search : '';
          ...
          {loading && state.products.length === 0 ? (
            <div id="catalog-skeleton-wrapper">
              <Skeleton
                ...
                count={9}
                wrapper={({ children }) => (
                  <div className="col-lg-4 col-sm-6 col-6 col-md-6">{children}</div>
                )}
              />
            </div>
            ...
            <InfiniteScroll
              ...
              initialLoad={false}
              className={s.right}>
              <ProductsList products={state.products} catalog={true} loading={loading} />
              {loading && <Loader />}
            </InfiniteScroll>
          )}
        </div>
