...
const CartMain = ({
  ...
  const [myCoupon, setMyCoupon] = useState(
    typeof window !== 'undefined' ? JSON.parse(localStorage.getItem('coupon')) : ' ',
  );
  ...
  if (myCoupon && myCoupon.amount) {
    switch (myCoupon.discountType) {
      case 'FIXED_CART':
        ...
        break;
      case 'PERCENT':
        orderReviewData.sale += (orderReviewData.totalPrice * myCoupon.amount) / 100;
        orderReviewData.totalPrice = Math.round(
          orderReviewData.totalPrice - (orderReviewData.totalPrice * myCoupon.amount) / 100,
        );
  ...
  const wishlistItem = wishlistItems.filter((wishlistItem) => wishlistItem.id === cartItems.id)[0];
          ...
          {active ? (
            <div className={s.coupon}>
              <div className={s.left}>
                ...
                Промокод <div className={s.name}> {myCoupon.code} </div> активирован!
              </div>
            ...
            <>
              ...
              <div className={s.coupon}>
                ...
                <div className={s.right}>
                  <button className={s.active} onClick={() => setActiveCoupon(true)}>
                    Активировать
                  </button>
                </div>
              </div>
                  ...
                  <div className={s.details}>
                    ...
                    <div className={s.cardBottom}>
                      <div className={s.remove}>
                        <div dangerouslySetInnerHTML={{ __html: icons.remove }} />
                        <button onClick={() => deleteFromCart(product.selectedProductId)}>
                          Удалить
                        </button>
                      </div>
                      <div className={`${s.wishlist} ${wishlistItem ? s.active : null}`}>
                        <button
                          onClick={
                            wishlistItem
